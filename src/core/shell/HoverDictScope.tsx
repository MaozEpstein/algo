...
export default function HoverDictScope({ children }: { children: ReactNode }) {
  ...
  const index = useMemo(() => {
    ...
    for (const lec of course.LECTURE_LIST)
      for (const g of lec.glossary ?? []) {
        const plain = g.term.replace(/\$[^$]*\$/g, ' ').toLowerCase()
        for (const tok of plain.split(/[^֐-׿a-z]+/)) if (tok.length >= 3 && !STOP.has(tok) && !m.has(tok)) m.set(tok, { term: g.term, def: g.def })
      }
    return m
  }, [on, course])
  ...
  const probe = (x: number, y: number) => {
    const w = wordAt(x, y)
    const hit = w ? index.get(w) : undefined
    if (hit && scopeRef.current) {
