...
export const idealDiodeLecture: LectureModule = {
  ...
  glossary: [
    ...
    { term: 'דיודה ארוכה / קצרה', def: 'לפי רוחב הבסיס הניטרלי מול $L$: ארוכה ($W_B\\gg L$) — פרופיל מעריכי, זרם מוגבל-רקומבינציה; קצרה ($W_B\\ll L$) — פרופיל לינארי, זרם מוגבל-מעבר וגדול יותר. הזרם נקבע מאורך אפקטיבי $L_{eff}=L\\tanh(W_B/L)$.', tex: 'L_{eff}=L\\tanh(W_B/L)' },
  ],
  formulas: [
    ...
    { name: 'דיודה קצרה — אורך אפקטיבי', tex: 'L_{eff}=L\\tanh\\!\\left(\\tfrac{W_B}{L}\\right),\\quad J_S\\propto 1/L_{eff}', note: 'ארוכה $\\to L$, קצרה $\\to W_B$ (זרם גדול יותר).' },
  ],
  symbols: [
    ...
    { sym: 'L_{eff}', he: 'אורך דיפוזיה אפקטיבי, $L\\tanh(W_B/L)$', unit: 'cm' },
  ],
}
