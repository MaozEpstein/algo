...
const QA: { q: ReactNode; a: ReactNode }[] = [
  ...
  {
    q: <>חישוב: מהו רוחב-הדלדול המרבי <Tex>{'W_{max}'}</Tex> בהיפוך (<Tex>{'\\psi_s=2\\phi_F=0.81\\,V'}</Tex>, <Tex>{'N_A=10^{17}'}</Tex>)?</>,
  ...
  },
  {
    ...
    a: <>מאחר ש-<Tex>{'V_{FB}<0<V_T'}</Tex> → <b>דלדול</b>. (מתחת ל-<Tex>{'V_{FB}'}</Tex> הצטברות; מעל <Tex>{'V_T'}</Tex> היפוך.)</>,
  },
  {
  ...
  {
    q: <>בהיפוך, מדוע רוחב-הדלדול מפסיק לגדול (<Tex>{'W=W_{max}'}</Tex>)?</>,
    a: <>ברגע שנוצר ערוץ-ההיפוך, כל תוספת מטען-שער מאוזנת ע״י <b>אלקטרוני-ההיפוך</b> (לא ע״י הרחבת הדלדול). לכן <Tex>{'\\psi_s'}</Tex> ננעל על <Tex>{'2\\phi_F'}</Tex> ו-<Tex>{'W'}</Tex> נשאר מרבי.</>,
  },
]
